...
/*
let lock = 0; // 0 means unlocked, 1 means locked

// Function representing a process trying to enter the critical section
function enterCriticalSection(processId) {
    if (lock === 0) { // Check if lock is available
        console.log(`Process ${processId} is entering the critical section.`);
        lock = 1; // Acquire the lock

        // Simulate process working in the critical section
        setTimeout(() => {
            exitCriticalSection(processId);
        }, Math.random() * 2000 + 1000); // Random delay to simulate work
    } else {
        console.log(`Process ${processId} is waiting, as the critical section is locked.`);
        setTimeout(() => {
            enterCriticalSection(processId); // Retry after some delay
        }, 500);
    }
}

// Function representing a process exiting the critical section
function exitCriticalSection(processId) {
    console.log(`Process ${processId} is leaving the critical section.`);
    lock = 0; // Release the lock
}

// Simulation of multiple processes attempting to access the critical section
function simulateProcesses(processCount) {
    for (let i = 1; i <= processCount; i++) {
        enterCriticalSection(i);
    }
}

// Start the simulation with a specified number of processes
simulateProcesses(3);
*/
